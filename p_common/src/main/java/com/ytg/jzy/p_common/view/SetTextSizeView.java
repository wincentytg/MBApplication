...
public class SetTextSizeView extends View {
    ...
    private  final String TAG="SetTextSizeView";
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        lineWidth = DensityUtil.dip2px(context, 2);
        ...
        currentProgress = TextUtil.getCurrentPoint();
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        lineHeight = height / 4;
        // 横线宽度是总宽度-2个圆的半径
        itemWidth = (w - 2 * circleRadius-5) / max;
        // 把可点击点保存起来
        for (int i = 0; i <= max; i++) {
            points.add(new Point(circleRadius + i * itemWidth, height / 2));
        }
        if(onItemSizeListener!=null){
            onItemSizeListener.onItemSize(itemWidth);
        }


//        LogUtil.d(TAG,"itemWidth--"+itemWidth+"lineHeight--"+lineHeight+"circleRadius--"+circleRadius+"");
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (canMove) {
            ...
            // 最终
            if(TextUtil.getCurrentPoint()!=-1){
                circleX = points.get(TextUtil.getCurrentPoint()).x;
                if(TextUtil.getCurrentPoint()==0){
                    circleX=circleX+2;
                }
            }else{
